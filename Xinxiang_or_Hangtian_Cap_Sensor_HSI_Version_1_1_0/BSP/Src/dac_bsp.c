...
* - DAC值
*/
uint16_t BSP_Get_DAC(DA_CHANNEL_NUM DA_Channel_Num)
{
    uint32_t channel;
...
/**@brief       DAC设备读数据,本函数被rt_device_register注册后,
* 被rt_device_read调用
* @param[in]    dev : 设备句柄
* @param[in]    pos : 读取数据偏移量;
* @param[in]    buffer : 内存缓冲区指针，读取的数据将会被保存在缓冲区中;
* @param[in]    size : 读取数据的大小
* @return       函数执行结果
* - size: 读取数据的实际大小，如果是字符设备，返回大小以字节为
* 单位，如果是块设备，返回的大小以块为单位;
* - 0: 需要读取当前线程的 errno 来判断错误状态
*/
static rt_size_t stm32_dac_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    uint8_t i;
    ...
    for(i = pos; i < (size + pos); i++)
    {
        if(i >= DA_CHANNEL_TOTAL)
            return i - pos;
        *(uint16_t *)buffer = BSP_Get_DAC((DA_CHANNEL_NUM)i);
    }
    return size;
}
...
static int dac_device_init(void)
{
    ...
    dac_device_obj.read     = stm32_dac_read;
